...
void ReadPotMeters() {
  ...
  servoMinPulse = map(potInput, MIN_VALUE_MEASURED_FOR_POT, MAX_VALUE_MEASURED_FOR_POT, SERVO_MIN_POSITION, SERVO_MIDDLE_POSITION);
  servoMaxPulse = SERVO_MIDDLE_POSITION + (SERVO_MIDDLE_POSITION - servoMinPulse);
  Serial.print("Servo min and max pulse: "); Serial.print(servoMinPulse);Serial.print(servoMaxPulse);
}
...
void SetMagnetLEDs(int potInput) {
  ...
  if (potInput > CHANNEL_CENTER) {
    ...
    //scale right down
    output = map(abs(potInput - CHANNEL_CENTER), 0, CHANNEL_CENTER, 0, 255);
